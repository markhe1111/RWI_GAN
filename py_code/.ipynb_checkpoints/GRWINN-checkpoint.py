...
def GRWINN  (data_x, data_m,  parameters):
    ...
    # make negative to have concave
     
    D_loss = -tf.reduce_mean(M * tf.log(D_hat + 1e-8) + (1-M) * tf.log(1. - D_hat + 1e-8)) 
    #D_loss = -tf.reduce_mean(M * tf.log(D_hat + 1e-8) + (1-M) * tf.log(1. - D_hat + 1e-8)) 

     
    dQ_ob   =   tf.map_fn(lambda j: ob_adj_qtl((X, D_hat, M), j), 
                          p_range , fn_output_signature=tf.float32) 
    dQ_msg  =   tf.map_fn(lambda j: msg_adj_qtl( (G_sample, 1-D_hat, M), j), 
                          p_range , fn_output_signature=tf.float32) 
    
    OMDiff =  tf.abs(  dQ_msg -dQ_ob ) 
    Q_loss  = tf.reduce_sum(OMDiff)   
    
    
    D_solver = tf.train.AdamOptimizer().minimize(D_loss, var_list=theta_D)
    G_solver = tf.train.AdamOptimizer().minimize(Q_loss, var_list=theta_G)
    ...
    for it in tqdm(range(1,ITER)):    
        ...
        _, Q_loss_curr = sess.run([G_solver, Q_loss], feed_dict = {X: X_mb, M: M_mb})

        # NOTE i changed D_loss_temp
        Med_loss_record[it] = Q_loss_curr
    ...
    imputed_Mask =  sess.run([D_real], feed_dict = {X: norm_data_x, M: data_m})[0]    

    X_hat = data_m * data_0 + (1-data_m) * norm_impute 
 
    return X_hat, Med_loss_record, D_loss_record,imputed_Mask
